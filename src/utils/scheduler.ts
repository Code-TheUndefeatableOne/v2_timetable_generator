...
export function autoGenerateTimetable(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    simultaneousConstraints.filter(sim => sim.isAlways).forEach(sim => {
      ...
      // Pass 1: Try to spread across days (max 1 per day)
      for (const day of shuffledDays) {
        ...
        for (const pIdx of shuffledPeriods) {
          ...
          if (allFree) {
            ...
            placedCount++;
            break; // Move to next day in Pass 1
          }
        }
      }
      ...
      if (placedCount < periodsNeeded) {
        for (const day of shuffledDays) {
          if (placedCount >= periodsNeeded) break;
          const dayPlacedCount = currentEntries.filter(e => e.sectionId === firstPair.sectionId && e.subjectId === firstPair.subjectId && e.day === day).length;
          if (dayPlacedCount >= (sub?.maxPeriodsPerDay || 1)) continue;
          ...
          for (const pIdx of shuffledPeriods) {
            if (placedCount >= periodsNeeded) break;
            if (currentEntries.filter(e => e.sectionId === firstPair.sectionId && e.subjectId === firstPair.subjectId && e.day === day).length >= (sub?.maxPeriodsPerDay || 1)) break;
    ...
    electiveGroups.forEach(eg => {
      ...
      // Pass 1: Spread across days (max 1 per day)
      for (const day of shuffledDays) {
        ...
        for (const pIdx of shuffledPeriods) {
          if (placedCount >= eg.periodsPerWeek) break;
          if (currentEntries.filter(e => e.electiveGroupId === eg.id && e.day === day).length >= 1) continue;
      ...
      if (placedCount < eg.periodsPerWeek) {
        for (const day of shuffledDays) {
          ...
          for (const pIdx of shuffledPeriods) {
            if (placedCount >= eg.periodsPerWeek) break;
            if (currentEntries.filter(e => e.electiveGroupId === eg.id && e.day === day).length >= eg.maxPeriodsPerDay) break;
    ...
    sectionsToProcess.forEach(section => {
      section.assignments.forEach(a => {
        ...
        if (remaining > 0) {
          // Difficulty = (Weekly Load / Daily Max) * (Teacher's Total Load)
          // Add a penalty for subjects that are part of ANY constraint (even if not Always)
          const isConstrained = syncConstraints.some(sync => sync.subjectId === a.subjectId) || 
                               simultaneousConstraints.some(sim => sim.sectionSubjectPairs.some(p => p.subjectId === a.subjectId));
          
          const difficulty = (sub.maxPeriodsPerWeek / sub.maxPeriodsPerDay) * 
                            (teacherTotalLoad[a.teacherId] || 1) * 
                            (isConstrained ? 1.5 : 1);
