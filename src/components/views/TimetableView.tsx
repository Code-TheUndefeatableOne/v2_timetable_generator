      ...
      <div className="flex items-start justify-between gap-1">
        <div className="flex items-center gap-1.5 min-w-0">
          <div 
            {...listeners} 
            {...attributes}
            className="cursor-grab active:cursor-grabbing p-0.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded transition-colors print:hidden"
          >
...
export const TimetableView: React.FC<TimetableViewProps> = ({
  ...
  const exportToPDF = async () => {
    ...
    // Helper to clean oklch colors from style attributes before capture
    const cleanStyles = (root: HTMLElement) => {
      const elements = root.querySelectorAll('*');
      elements.forEach(el => {
        const htmlEl = el as HTMLElement;
        // Remove all shadows and rings that cause "thick" look in PDF
    ...
    try {
      if (viewMode === 'teacher') {
        ...
        for (let i = 0; i < teacherCards.length; i++) {
          ...
          const renderWidth = pdfWidth - 20; // Margin
          const renderHeight = renderWidth / ratio;

          if (i > 0) pdf.addPage();
          
          // Center vertically if it fits
          const yPos = renderHeight < pdfHeight ? (pdfHeight - renderHeight) / 2 : 10;
          pdf.addImage(dataUrl, 'PNG', 10, yPos, renderWidth, renderHeight, undefined, 'FAST');
        ...
        pdf.addImage(dataUrl, 'PNG', 10, 10, renderWidth, renderHeight, undefined, 'FAST');
      ...
      // Ensure class is removed from all possible elements
      document.querySelectorAll('.exporting-pdf').forEach(el => el.classList.remove('exporting-pdf'));
  ...
  return (
    ...
    <style>{`
        ...
        .exporting-pdf .teacher-name, .exporting-pdf .section-name {
          font-size: 9pt !important;
          font-weight: normal !important;
          display: block !important;
          margin-top: 4px !important;
        }
        /* Swap for teacher view export */
        .teacher-card.exporting-pdf .section-name {
          font-size: 11pt !important;
          font-weight: bold !important;
        }
        .teacher-card.exporting-pdf .subject-name {
          font-size: 9pt !important;
          font-weight: normal !important;
          margin-top: 4px !important;
        }
        ...
        .exporting-pdf .mt-6.pt-6 {
          display: none !important; /* Hide summary stats in export */
        }
        @keyframes spin-slow {
