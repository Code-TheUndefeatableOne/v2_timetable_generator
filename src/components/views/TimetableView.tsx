...
const DraggableEntry: React.FC<DraggableEntryProps> = ({ 
  ...
  return (
    <div 
      ...
      style={style}
      className={cn(
        "h-full flex flex-col justify-between transition-all relative z-10 group",
        isComparison ? "p-1 md:p-1.5" : "p-2",
    ...
    >
      <div className="flex items-start justify-between gap-1">
        <div className="flex items-center gap-1.5 min-w-0">
          <div 
            {...listeners} 
            {...attributes}
            className="cursor-grab active:cursor-grabbing p-0.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded transition-colors print:hidden no-print grip-handle"
          >
            <GripVertical size={10} className="text-zinc-400" />
          </div>
          <div className="w-1.5 h-1.5 rounded-full shrink-0 print:hidden no-print" style={{ backgroundColor: isElective ? '#EAB308' : subject?.color }} />
          <span className={cn(
            "subject-name font-bold text-zinc-900 dark:text-zinc-100 truncate",
            isComparison ? "text-[8px] md:text-[9px]" : "text-[10px]"
          )}>
            {isElective ? (electiveGroup?.name || 'Elective') : subject?.name}
          </span>
        </div>
        <div className="entry-controls flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all print:hidden no-print">
          <button 
            onClick={(e) => {
              e.stopPropagation();
              onDelete();
            }}
            className="p-1 text-zinc-300 hover:text-rose-500 transition-all print:hidden no-print"
          >
            <Trash2 size={10} />
          </button>
        </div>
      ...
      <div className={cn(
        "mt-1 flex items-center gap-1 text-zinc-400 font-medium print:text-zinc-900 print:text-[10px] print:font-bold",
        isComparison ? "text-[7px] md:text-[8px]" : "text-[9px]"
      )}>
        <User size={isComparison ? 8 : 10} className="shrink-0 print:hidden no-print" />
...
const DroppableCell: React.FC<DroppableCellProps> = ({ day, periodIndex, sectionId, children, isOver, isDragging, validation, isComparison }) => {
  ...
  return (
    <div 
      ref={setNodeRef}
      className={cn(
        "h-full w-full transition-all relative group flex flex-col",
        isComparison ? "min-h-[50px]" : "min-h-[80px]",
...
export const TimetableView: React.FC<TimetableViewProps> = ({
  ...
  const renderTimetableCard = (sectionId: string, isComparison: boolean = false) => {
    ...
    return (
      <motion.div 
        ...
        className={cn(
          "bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden print:border-none print:shadow-none flex flex-col w-full",
          isComparison ? "max-h-[380px] xl:max-h-[38vh] xl:min-h-[300px] shadow-2xl" : "max-h-[70vh]"
        )}
      >
        <div className="p-3 border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/50 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-1.5 print:hidden no-print">
            <Button 
              variant="outline" 
              size="sm" 
              onClick={() => onAutoGenerateForSection(sectionId, true)}
              className="h-7 px-2 text-[8px] font-black uppercase tracking-widest text-emerald-600 border-emerald-100 hover:bg-emerald-50 dark:border-emerald-900/30 dark:hover:bg-emerald-900/20 print:hidden no-print"
            >
              <Plus size={10} className="mr-1 print:hidden no-print" /> Fill Gaps
            </Button>
            <Button 
              variant="outline" 
              size="sm" 
              ...
              className="h-7 w-7 p-0 text-rose-500 border-rose-100 hover:bg-rose-50 dark:border-rose-900/30 dark:hover:bg-rose-900/20 print:hidden no-print"
            >
              <Trash2 size={12} className="print:hidden no-print" />
            </Button>
          </div>
        </div>
          ...
          <table className="w-full border-collapse table-fixed min-w-[600px]">
            <thead className="sticky top-0 z-20 bg-zinc-50 dark:bg-zinc-800">
              <tr>
                <th className={cn(
                  ...
                  isComparison ? "w-12 p-1" : "w-16 p-2"
                )}></th>
                {structure.slots.map((slot, sIdx) => (
                  <th 
                    key={slot.id} 
                    className={cn(
                      "border-b border-zinc-100 dark:border-zinc-800 text-center",
                      isComparison ? "p-1 md:p-1.5" : "p-2",
                  ...
                  >
                    <div className="flex flex-col items-center gap-0.5">
                      <span className={cn(
                        "font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500",
                        isComparison ? "text-[7px]" : "text-[8px]"
                      )}>
                        {slot.label}
                      </span>
                      <span className={cn(
                        "font-medium text-zinc-400 uppercase tracking-tighter",
                        isComparison ? "text-[6px]" : "text-[7px]"
                      )}>
                        {slot.startTime}
                      </span>
                    </div>
            ...
            <tbody>
              {DAYS.map(day => (
                <tr key={day}>
                  <td className={cn(
                    "border-r border-zinc-100 dark:border-zinc-800 bg-zinc-50/20 dark:bg-zinc-800/10 sticky left-0 z-10 bg-white dark:bg-zinc-900",
                    isComparison ? "p-1 md:p-1.5" : "p-2"
                  )}>
                  ...
                  {structure.slots.map((slot, sIdx) => {
                    ...
                    return (
                      <td 
                        ...
                        className={cn(
                          "p-0 border-r border-b border-zinc-100 dark:border-zinc-800 last:border-r-0 relative group transition-all duration-500",
                          isComparison ? "min-w-[100px]" : "min-w-[150px]"
                        )}
                      >
                        <DroppableCell 
                          day={day} 
                        ...
                        >
                          {groupedEntries.length > 0 ? (
                            ...
                            <div className={cn(
                              "h-full w-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity",
                              isComparison ? "min-h-[50px]" : "min-h-[80px]"
                            )}>
  ...
  const handleResolveConflicts = async () => {
    ...
    try {
      ...
      // Helper to get start and end times for a specific period in a section's schedule
      const getSlotTimes = (secId: string, pIndex: number) => {
        const sec = sections.find(s => s.id === secId);
        if (!sec) return null;
        const struct = Array.isArray(gradeStructures) 
          ? gradeStructures.find(g => g.id === sec.gradeId) 
          : gradeStructures[sec.gradeId];
        if (!struct) return null;
        const periodSlots = struct.slots.filter(s => s.type === 'period');
      ...
      // Group entries by teacher, day, and period to find overlaps
        ...
        for (let i = 1; i < conflictList.length; i++) {
          ...
          for (const day of DAYS) {
            for (let pIdx = 0; pIdx < structure.slots.filter(s => s.type === 'period').length; pIdx++) {
              ...
              const targetTimes = getSlotTimes(entryToMove.sectionId, pIdx);
              const isTeacherFree = targetTimes ? !currentEntries.some(e => {
                if (e.teacherId !== entryToMove.teacherId || e.day !== day) return false;
                // Exclude the entry we are moving itself
                if (e.id === entryToMove.id) return false;
                
                const otherTimes = getSlotTimes(e.sectionId, e.periodIndex);
                if (!otherTimes) return false;
                
                // Real-time overlap condition: start1 < end2 && start2 < end1
                return targetTimes.start < otherTimes.end && otherTimes.start < targetTimes.end;
              }) : false;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over) {
      ...
      if (activeData?.type === 'grid-entry') {
        ...
        // Automatically clean up/replace any existing single or non-simultaneous entry in the same slot for this section to prevent overlap
                  ...
                  {selectedSectionIds.length > 1 && (
                    ...
                    >
                      ...
                      >
                        ...
                        {/* Comparison Grid */}
                        <div className="p-4 md:p-6 overflow-auto custom-scrollbar flex-1 bg-zinc-50/30 dark:bg-zinc-900/30 max-h-[calc(90vh-100px)]">
                          <div className={cn(
                            "grid gap-4 md:gap-6 w-full h-full justify-items-center items-stretch",
                            selectedSectionIds.length === 1 ? "grid-cols-1" :
                            selectedSectionIds.length === 2 ? "grid-cols-1 lg:grid-cols-2" :
