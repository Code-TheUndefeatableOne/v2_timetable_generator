...
export const SubjectsView: React.FC<SubjectsViewProps> = ({ subjects = [], setSubjects, searchQuery = '' }) => {
  const addSubject = () => {
    ...
    // List of distinct, professional colors
    const palette = [
      '#6366f1', '#ec4899', '#f59e0b', '#10b981', '#3b82f6', 
      '#8b5cf6', '#f43f5e', '#06b6d4', '#84cc16', '#f97316',
      '#a855f7', '#14b8a6', '#ef4444', '#0ea5e9', '#22c55e',
      '#64748b', '#78350f', '#1e3a8a', '#4c1d95', '#701a75',
      '#451a03', '#064e3b', '#164e63', '#1e1b4b', '#312e81'
    ];
    
    // Find a color that isn't used yet, or pick a random one if all are used
    const usedColors = subjects.map(s => s.color);
    const availableColor = palette.find(c => !usedColors.includes(c)) || palette[Math.floor(Math.random() * palette.length)];
