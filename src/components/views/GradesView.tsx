...
export const GradesView: React.FC<GradesViewProps> = ({ 
  ...
  const duplicateGrade = (grade: GradeStructure) => {
    const newGradeId = Math.random().toString(36).substr(2, 9);
    
    // Duplicate grade structure
    setGradeStructures([...gradeStructures, { 
      ...grade, 
      id: newGradeId, 
      name: `${grade.name} (Copy)`,
      slots: grade.slots.map(s => ({ ...s, id: Math.random().toString(36).substr(2, 9) }))
    }]);

    // Duplicate sections for this grade
    const gradeSections = sections.filter(s => s.gradeId === grade.id);
    const newSections = gradeSections.map(s => ({
      ...s,
          ...
          <div key={grade.id} className="space-y-8 bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-zinc-100 dark:border-zinc-800 shadow-sm transition-colors">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => duplicateGrade(grade)}>
                  <Layers size={14} className="mr-2" /> Duplicate Structure
                </Button>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                        <button 
                          ...
                          className="p-2 text-zinc-300 hover:text-gold-500 transition-all"
