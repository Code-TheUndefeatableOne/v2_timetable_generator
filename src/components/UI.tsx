...
export const Badge = ({ children, variant = 'default', className, title, ...props }: React.HTMLAttributes<HTMLSpanElement> & { variant?: 'default' | 'success' | 'warning' | 'error' }) => {
  const variants = {
  ...
  return (
    <span 
      title={title}
      {...props}
      className={cn("px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-widest inline-flex items-center", variants[variant], className)}
    >
      {children}
    </span>
  );
